...
void CWolf::Update()
{
	...
	case EState::EDEATH: //éÄñSéû
		//HPÇ™0Ç…Ç»Ç¡ÇΩêîïbå„Ç…è¡ñ≈Ç≥ÇπÇÈ
		if (mWolfTime3 == 19)
		{
	...
	case EState::EATTACK: //çUåÇéû
		...
		if (mWolfTime2 == 4)
		{
			if (mWVx < 0)
			{
				...
				mpWolfAttackBox = new CWolfAttackBox(X() - 125, Y() + 40, 60.0f, 40.0f, CWolf::Texture7());
				mWolfEattack = 4;
			}
			if (mWVx > 0)
			{
	...
	case EState::EMOVE:
		...
		{
			if (Y() > CPlayer::Instance()->Y() - 25 && Y() < CPlayer::Instance()->Y() + 25)
			{
				if (mWolfTime2 <= 0)
				{
					mWolfTime2 = 30;
